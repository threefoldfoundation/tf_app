...
import { getNodesStats, getNodesStatsStatus } from '../../state/app.state';
...
export class NodeStatusPageComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.nodes$ = this.store.pipe(select(getNodesStats));
    this.status$ = this.store.pipe(select(getNodesStatsStatus));
  }
